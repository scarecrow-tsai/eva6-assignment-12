...
The code below implements the self-attention mechanism with multiple heads. 
It creates 3 vectors `q`, `k`, and `v`. Then it matrix multiplies `q` and `k`, normalizes it and performs softmax on it. 

These softmax attention scores are then matrix multiplied with the `v` vector to obtain the final attention output. 
